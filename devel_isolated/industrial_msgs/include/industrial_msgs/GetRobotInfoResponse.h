...
struct GetRobotInfoResponse_
{
  ...
  _controller_type controller;
   ...
   typedef std::vector< ::industrial_msgs::DeviceInfo_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::industrial_msgs::DeviceInfo_<ContainerAllocator> >> _robots_type;
  _robots_type robots;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::industrial_msgs::GetRobotInfoResponse_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.robots.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::industrial_msgs::DeviceInfo_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.robots[i]);
    }
    if (v.robots.empty() || false)
      s << "]";
    if (true || !indent.empty())
